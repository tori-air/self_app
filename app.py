...
def custom_weight_1(U, v, data):
    ...
    elif "natural" in data and data["natural"] in ["water", "wetland"]:
        return length * 0.7  # 川沿いの道は少し優先
    elif "leisure" in data and data["leisure"] in ["park", "nature_reserve"]:
...
def custom_weight_2(U, v, data):
    ...
    elif shop or amenity:  # 店や施設があるエリアを優先
        return length * 0.5  # 商業施設や商店街のルートを優先
    # 主要道路はコストを大きくして避ける
    elif highway in ["primary", "secondary", "primary_link", "secondary_link"]:
        return length * 10  # 主要道路は避ける
    # 他の道路は通常通り
    else:
        return length
